...
public class GameManager : MonoBehaviour
{
    ...
    void LoadGameData()
    {
        ...
        //killCount = PlayerPrefs.GetInt("KILL_COUNT", 0);       // PlayerPrefs.GetInt(string key, int initNum)      // initNum은 key 값이 존재하지 않았을 때 반환할 초기값이다.
    ...
    // f로드한 데이터를 기준으로 인벤토리에 아이템을 추가하는 함수
    void InventorySetup()
    {
        ...
        for(int i = 0; i<gameData.equipItem.Count; ++i)
        {
            ...
            for(int j = 1; j>slots.Length; j++) {
                ...
                // 아이템의 ItemInfo 클래스의 itemDat에 로드한 데이터 값을 저장
    ...
    public GameObject GetBullet()
    {
        for (int i = 0; i < bulletPool.Count; ++i)
        {
            ...
            if(bulletPool[i] != null && bulletPool[i].activeSelf == false)
            {
                return bulletPool[i];
            }
        }
    ...
    public void IncKillCount()
    {
        ...
        killCountTxt.text = "KILL " + gameData.killCount.ToString("0000");
        // 죽인 횟수를 저장
        //PlayerPrefs.SetInt("KILL_COUNT", gameData.killCount);            //PlayerPrefs.SetInt(string key , int val);
    }
